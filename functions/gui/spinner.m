...
%  property values.
%  Properties:
%    Position : position rectangle, default [0 0 0.2 0.2]
%    Unit: unit of position, either normalized or pixels.
    ...
    switch ind
        ...
        case 7 %Callback
         ...
         case 8 %Tag
            ...
            tag = val;
        case 9 % Enable
            if max(strcmp(val, {'on','off'}))
                enable = val;
            else
                error('Value for Enable must be either off or on.');
            end
        case 10 % Unit
            if max(strcmp(val, {'normalized','pixels'}))
                unit = val;
            else
                error('Value for Unit must be ''normalized'' or ''pixels''.');
            end
        otherwise
...
for i=0:mid/4
    start=mid-i;
    last=mid+i;
    push_cdata(i+7,start:last,:) = 0;
end


% create uicontrols
h_edit = uicontrol('Units',unit,...
                 'Position',pos,...
                 'Style','edit',...
                 'Tag',[tag '_edit'],...
...
h_down = uicontrol('Units',unit,...
                 'Position',[pos(1) + (pos(3) - push_width) - offs, pos(2) + offs, push_width, push_height - offs],...
                 'CData',flipdim(push_cdata,1),...
                 'Tag',[tag '_down'],...
                 'Parent',parent,...
                 'SelectionHighlight','off');
h_up = uicontrol('Units',unit,...
                 'Position',[pos(1) + (pos(3) - push_width) - offs, pos(2) + push_height, push_width, pos(4) - push_height - offs],...
                 'CData',push_cdata,...
                 'Tag',[tag '_up'],...
                 'Parent',parent,...
                 'SelectionHighlight','off');
...
if ~isempty(get(s.edit,'String'))
    ...
    elseif iscell(s.callback) 
        if numel(s.callback) >= 3
        s.callback{2} = s.edit; % edit box handle
        s.callback{3} = s; % spinner eventdata with values
        feval(s.callback{:});
        else
            error('Spinner Callback Function must possess at least 3 input parameters. The first element must be a string with the function name.');
        end
    end
end
...
%if edit is disabled, do nothing
if strcmp(s.edit.Enable, 'off')
   return;
end
